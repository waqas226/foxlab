...
$(function () {
  ...
  var dt_todo = $('.datatables-todo').DataTable({
    ...
    serverSide: true,
    ajax: {
      url: '/manage-renewals/show?company_id=select',
      type: 'GET',
      data: function (d) {
        d._token = '{{ csrf_token() }}';
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, full) {
          ...
          <a href="javascript:;" class="text-warning transfer-record" title="Send to todo" data-id="${full.id}"><i class="ti ti-file-symlink"></i></a>
        `;
        }
      }
    ],
  ...
  $('.datatables-todo tbody').on('click', 'a.text-primary', function () {
    ...
    $rowNode
      .find('td')
      .eq(3 - ind)
      .html(`<input type="text" class="form-control " id="flatpickr-validation" value="${rowData.renewal_date}" />`);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          error: function (xhr) {
            toastr.error('Error deleting renewals', 'Error', {
              closeButton: true,
              progressBar: true,
              timeOut: 2000,
              positionClass: 'toast-top-right'
  ...
  $('.datatables-todo tbody').on('click', 'a.transfer-record', function () {
    ...
    $.ajax({
      url: `/manage-renewals/transfer/${id}`,
      type: 'GET',
      data: {
        _token: $('meta[name="csrf-token"]').attr('content')
      },
      ...
      error: function (xhr) {
        toastr.error('Error while trnasfering renewals', 'Error', {
          closeButton: true,
          progressBar: true,
          timeOut: 2000,
          positionClass: 'toast-top-right'
  ...
  $('.print-todo').on('click', function () {
    ...
    var url = '/manage-renewals/print/' + company_id;
  ...
  const fv = FormValidation.formValidation(addNewTodoForm, {
    fields: {
      ...
      title: {
        validators: {
          notEmpty: {
            message: 'Please enter renewal title'
          }
        }
      },
      renewal_date: {
