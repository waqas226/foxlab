...
$(function () {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $sts = 'A';
            if (full['status'] == 'A') {
              $txt = 'Deactivate this Role';
              $sts = 'D';
            } else {
              $txt = 'Activate this Role';
              $sts = 'A';
            }
            return (
  ...
  $('.datatables-company tbody').on('click', '.status-update', function () {
    ...
    var id = $this.data('id');
    $.ajax({
      url: '/manage-roles/update-status',
      type: 'POST',
      headers: {
  ...
  $('.datatables-company tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        // Ajax request
        $.ajax({
          url: '/manage-roles/delete/' + $this.data('id'),
          type: 'GET',
          success: function (response) {
            if (response.error) {
              Swal.fire({
